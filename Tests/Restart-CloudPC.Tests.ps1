...
Describe 'Restart-CloudPC' {
    ...
    It 'POSTs to the reboot endpoint for the given Id' {
        Restart-CloudPC -Id 'cpc-1' -Force -Confirm:$false

        Should -Invoke -ModuleName WindowsCloudPC Invoke-MgGraphRequest -Times 1 -Exactly -ParameterFilter {
            $Method -eq 'POST' -and $Uri -like '*cloudPCs/cpc-1/reboot'
        }
    }
    ...
    It 'accepts WindowsCloudPC.CloudPC objects from the pipeline' {
        ...
        $cpc | Restart-CloudPC -Force -Confirm:$false

        Should -Invoke -ModuleName WindowsCloudPC Invoke-MgGraphRequest -Times 1 -Exactly -ParameterFilter {
            $Uri -like '*cloudPCs/cpc-from-pipeline/reboot'
        }
    }

    It 'reboots every Cloud PC piped in' {
        $cpcs = @(
        ...
        $cpcs | Restart-CloudPC -Force -Confirm:$false
    ...
    It 'emits a RestartResult object with -PassThru' {
        $result = Restart-CloudPC -Id 'cpc-1' -PassThru -Force -Confirm:$false

        $result | Should -Not -BeNullOrEmpty
        $result.PSObject.TypeNames | Should -Contain 'WindowsCloudPC.RestartResult'
    ...
    It 'reports Failed status with -PassThru when Graph throws' {
        ...
        $result = Restart-CloudPC -Id 'cpc-broken' -PassThru -Force -Confirm:$false -ErrorAction SilentlyContinue
    ...
    It 'does not call Graph when -WhatIf is passed' {
        Restart-CloudPC -Id 'cpc-1' -WhatIf
